...
function clearAllFields() {
  document.getElementById("promptRole").value =
    getTranslation("default_papel_ia");
  document.getElementById("prompt1").value =
    getTranslation("default_instrucao");
...
function insertText(id, text) {
  ...
  const valueToKeyMap = {
    ...
    "Responda como se você estivesse respondendo uma criança de 10 anos. ":
      "crianca_10_anos",
    "Responda com uma To-Do list pois irei copiar e colar no notion ":
      "todo_list",

    // Prompt 3 (Atenção) mappings
    "Realce em negrito as partes  importantes da sua resposta. ":
      "realce_negrito",
    "Realce com ` ` as partes mais importantes da sua resposta. ":
      "realce_crase",
    "Caso necessite de mais informações não responda ainda e me pergunte o que precisa. ":
      "mais_informacoes",
    ...
    "Tenho conhecimento INTERMEDIÁRIO no assunto. ":
...
function updateCharCount() {
  ...
  document.getElementById("char-counter").innerText =
    `${getTranslation("caracteres")} ${totalChars}`;
}
...
function updateSelectOptions() {
  ...
  const selectRole = document.querySelector(
    "select[onchange=\"insertText('promptRole', this.value)\"]",
  );
  if (selectRole) {
  ...
  const select1 = document.querySelector(
    "select[onchange=\"insertText('prompt1', this.value)\"]",
  );
  if (select1) {
    ...
    const studyHelperPromptPt = document.getElementById(
      "study-helper-prompt-pt",
    );
    const studyHelperPromptEn = document.getElementById(
      "study-helper-prompt-en",
    );
    const imageRefinerPromptPt = document.getElementById(
      "image-refiner-prompt-pt",
    );
    const imageRefinerPromptEn = document.getElementById(
      "image-refiner-prompt-en",
    );
    ...
    if (currentLanguage === "en") {
      ...
      if (studyHelperPromptPt) studyHelperPromptPt.style.display = "none";
      if (studyHelperPromptEn) studyHelperPromptEn.style.display = "block";
      ...
      if (studyHelperPromptPt) studyHelperPromptPt.style.display = "block";
      if (studyHelperPromptEn) studyHelperPromptEn.style.display = "none";
  ...
  const select2 = document.querySelector(
    "select[onchange=\"insertText('prompt2', this.value)\"]",
  );
  if (select2) {
  ...
  const select3 = document.querySelector(
    "select[onchange=\"insertText('prompt3', this.value)\"]",
  );
  if (select3) {
  ...
  const select4 = document.querySelector(
    "select[onchange=\"insertText('prompt4', this.value)\"]",
  );
  if (select4) {
...
function updateTextareaDefaults() {
  ...
  if (
    promptRole &&
    (promptRole.value === "Papel da IA: " ||
      promptRole.value === "AI Role: " ||
      promptRole.value === "")
  ) {
  ...
  if (
    prompt1 &&
    (prompt1.value === "Instrução: " ||
      prompt1.value === "Instruction: " ||
      prompt1.value === "")
  ) {
  ...
  if (
    prompt2 &&
    (prompt2.value === "Formato da resposta: " ||
      prompt2.value === "Response format: " ||
      prompt2.value === "")
  ) {
  ...
  if (
    prompt3 &&
    (prompt3.value === "Atenção: " ||
      prompt3.value === "Attention: " ||
      prompt3.value === "")
  ) {
  ...
  if (
    prompt4 &&
    (prompt4.value === "Contexto: " ||
      prompt4.value === "Context: " ||
      prompt4.value === "")
  ) {
...
const translations = {
  "pt-BR": {
    ...
    realce_crase:
      "Realce com ` ` (crase) as partes mais importantes da sua resposta.",
    mais_informacoes:
      "Caso necessite de mais informações não responda ainda e me pergunte o que precisa.",
    ...
    // Popup content
    popup_papel_ia:
      'Defina o papel que a IA deve assumir antes de responder.\n\nExemplo:\n"Agora você é um especialista em nutrição."',
    popup_instrucao:
      'Dê uma instrução direta à IA sobre o que ela deve fazer.\n\nExemplos:\n"Me explique este código."\n"Deixe este código mais limpo e rápido."',
    popup_formato:
      'Defina um formato para a IA te responder. Pode ser uma resposta resumida, tabela, lista com bullet points, etc...\n\nExemplo:\n"Responda em formato de tabela."',
    popup_atencao:
      'Estabeleça critérios para a IA.\n\nExemplo:\n"Marque em negrito as partes mais importantes da sua resposta."',
    popup_contexto:
      'Dê o contexto para que a IA possa responder de forma mais precisa.\n\nExemplo:\n"Estou escrevendo um artigo científico."',
  ...
  en: {
    ...
    todo_list:
      "Answer with a To-Do list as I will copy and paste it into Notion.",

    // Select options - Attention
    realce_negrito: "Highlight important parts of your response in bold.",
    realce_crase:
      "Highlight the most important parts of your response with ` ` (backticks).",
    mais_informacoes:
      "If you need more information, don't answer yet and ask me what you need.",
    ...
    // Popup content
    popup_papel_ia:
      'Define the role the AI should take before responding.\n\nExample:\n"Now you are a nutrition expert."',
    popup_instrucao:
      'Give the AI a direct instruction about what it should do.\n\nExamples:\n"Explain this code to me."\n"Make this code cleaner and faster."',
    popup_formato:
      'Define a format for the AI to respond to you. It can be a brief response, table, bullet point list, etc...\n\nExample:\n"Answer in table format."',
    popup_atencao:
      'Establish criteria for the AI.\n\nExample:\n"Mark the most important parts of your response in bold."',
    popup_contexto:
      'Give the context so the AI can respond more accurately.\n\nExample:\n"I\'m writing a scientific article."',
...
function translatePage() {
  // Update labels
  document.querySelector('label[for="promptRole"]').textContent =
    getTranslation("papel_ia");
  document.querySelector('label[for="prompt1"]').textContent =
    getTranslation("instrucao");
  document.querySelector('label[for="prompt2"]').textContent =
    getTranslation("formato_resposta");
  document.querySelector('label[for="prompt3"]').textContent =
    getTranslation("atencao");
  document.querySelector('label[for="prompt4"]').textContent =
    getTranslation("contexto");

  // Update buttons
  document.querySelector(".clear-all-btn").textContent =
    getTranslation("apaga_tudo");
  document.querySelector(".copy-all-btn").textContent =
    getTranslation("copia_tudo");
